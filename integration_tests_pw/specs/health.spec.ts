...
test.describe('Health', () => {
  ...
  test.describe('All healthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), sarApi.stubPing(), tokenVerification.stubPing()])
    })
    ...
    test('Info is accessible', async ({ page }) => {
      ...
      const payload = await response.json()
      expect(payload.build.name).toBe('hmpps-subject-access-request-ui')
    })
  })

  test.describe('Some unhealthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), sarApi.stubPing(), tokenVerification.stubPing(500)])
    })
    ...
    test('Health check status is down', async ({ page }) => {
      ...
      expect(payload.components.tokenVerification.details.retries).toBe(2)
    })
  })
})
