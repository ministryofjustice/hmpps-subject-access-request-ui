...
const MAX_CASEREFERENCE_LENGTH = 20
...
export default class InputsController {
  ...
  static saveInputs(req: Request, res: Response): void {
    ...
    const dateFromError = InputsController.validateDateFrom(dateFrom)
    const dateToError = InputsController.validateDateTo(dateTo, dateFromError ? '' : dateFrom)
    const caseReferenceError = InputsController.validateCaseReference(caseReference)
  ...
  static validateCaseReference(caseReference: string): string {
    ...
    if (caseReference.length > MAX_CASEREFERENCE_LENGTH) {
      return `Case Reference must be ${MAX_CASEREFERENCE_LENGTH} characters or less`
    }
    return ''
  }
}
